...
void
check_blocked_thread (struct thread *t, void *aux)
{
...
thread_set_priority (int new_priority)
{
  ...
  if(list_empty(&thread_current()->locks) || new_priority > current_priority)
  {
...
{
  /* Not yet implemented. */
  return fp_to_near(mul_mixed(load_avg, 100));
}
...
{
  /* Not yet implemented. */
  return fp_to_near(mul_mixed(thread_current()->recent_cpu, 100));
}

void thread_increase_cpu (void) {
  ASSERT(thread_mlfqs);
  ASSERT(intr_context());
...
void thread_update_priority (struct thread* t) {
  ...
  t->priority = PRI_MAX - fp_towards_zero(t->recent_cpu / 4) - (t->nice * 2);
...
void thread_update_load_avg (void) {
  ...
  fixed_point cons = div_fp(int_to_fp(59), int_to_fp(60));
  load_avg = add_fp(mul_fp(load_avg, cons), div_mixed(int_to_fp(ready_threads_num), 60));
}
...
next_thread_to_run (void) 
{
  ...
  {
    struct list_elem *max_priority = list_max(&ready_list, thread_cmp_priority, NULL);
